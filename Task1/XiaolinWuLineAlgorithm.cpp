...
void XiaolinWuLineAlgorithm::DrawWuLine(double x0, double y0, double x1, double y1, PgmImage& image)
{
	...
	if (dy < dx)
	{
		...
		image.Plot(xpxl0, ypxl0, static_cast<int>(255 - FPart(y_beg) * xgap  * 255));
		image.Plot(xpxl0, ypxl0 + 1, static_cast<int>(FPart(y_beg) * xgap * 255));
		//image->Plot(xpxl0, ypxl0, 255);
		double intery = y_beg + gradient;

		// последняя точка
		int x_end = (x1 > plot_bound - 1) ? IPart(x1) : static_cast<int>(round(x1));
		double y_end = y1 + gradient * (x_end - x1);
		xgap = FPart(x1 + 0.5);
		int xpxl1 = x_end;
		int ypxl1 = IPart(y_end);
		image.Plot(xpxl1, ypxl1, static_cast<int>(255 - FPart(y_end) * xgap * 255));
		image.Plot(xpxl1, ypxl1 + 1, static_cast<int>(FPart(y_end) * xgap * 255));
		//image->Plot(xpxl1, ypxl1, 255);
		
		for (int x = xpxl0 + 1; x < xpxl1; x++)
		{
			//Верхняя точка
			image.Plot(x, IPart(intery), static_cast<int>(255 - FPart(intery) * 255));
			//Нижняя точка
			image.Plot(x, IPart(intery) + 1, static_cast<int>(FPart(intery) * 255));
	...
	else
	{
		...
		image.Plot(xpxl0, ypxl0, static_cast<int>(255 - FPart(x_beg) * ygap  * 255));
		image.Plot(xpxl0 + 1, ypxl0, static_cast<int>(FPart(x_beg) * ygap * 255));
		//image->Plot(xpxl0, ypxl0, 255);
		double interx = x_beg + gradient;

		// последняя точка
		int y_end = (y1 > plot_bound - 1) ? IPart(y1) : static_cast<int>(round(y1));
		double x_end = x1 + gradient * (y_end - y1);
		ygap = FPart(y1 + 0.5);
		int ypxl1 = y_end;
		int xpxl1 = IPart(x_end);
		image.Plot(xpxl1, ypxl1, static_cast<int>(255 - FPart(x_end) * ygap * 255));
		image.Plot(xpxl1 + 1, ypxl1, static_cast<int>(FPart(x_end) * ygap * 255));
		//image->Plot(xpxl1, ypxl1, 255);

		for (int y = ypxl0 + 1; y < ypxl1; y++)
		{
			//Верхняя точка
			image.Plot(IPart(interx), y, 255 - static_cast<int>(FPart(interx) * 255));
			//Нижняя точка
			image.Plot(IPart(interx) + 1, y, static_cast<int>(FPart(interx) * 255));
